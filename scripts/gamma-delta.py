...
# NM tag == Edit distance =  that is, the minimal number of one-nucleotide edits (substitutions, insertions and deletions) needed to transform the 
# read string into the reference string. Only present if SAM record is for an aligned read.
# Also number of changes necessary to make this equal the reference, excluding clipping
...
def findNM(samLine):
    ...
    return 0    # Retur 0 when no NM tag present
...
def save_map_info(mapRow, mapRef, ReadDict):
    ...
    if (newRatio >= ReadDict[ReadId][0]): 
        ...
        # Changing A1 -- > Taking the values from the SAM file
...
def load_sam_data(samDocs):
    ...
    for samfile in samDocs: # Loop through files in folder
        ...
        for row in sam:  # Loop through lines in sam file
            if row[0][0] != '@': # Avoiding headers -- > Taking the first item from the list of a row == the first character if the first item of the list.
